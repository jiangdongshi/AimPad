...
export function ForgotPassword() {
  ...
  const { resetPassword, sendCode, isLoading, error, clearError } = useAuthStore();
  ...
  const handleReset = async (e: React.FormEvent) => {
    ...
    if (password !== confirmPassword) {
      setLocalError(locale['auth.passwordMismatch']);
      return;
    }
    ...
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center px-4 py-8">
      <Card variant="elevated" className="w-full max-w-md p-8">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-gaming text-accent mb-2">{locale['auth.resetPasswordTitle']}</h1>
          <p className="text-text-muted text-sm">{locale['auth.resetPasswordSubtitle']}</p>
        </div>

        {success ? (
          <div className="text-center space-y-4">
            <div className="flex items-center gap-2 px-3 py-2 rounded-md bg-green-500/10 border border-green-500/30">
              <span className="text-sm text-green-400">{locale['auth.resetSuccess']}</span>
            </div>
            <Link to="/login" className="text-accent hover:text-accent-dark font-medium text-sm">
              {locale['auth.login']}
            </Link>
          </div>
        ) : (
          <form onSubmit={handleReset} className="space-y-5">
            ...
            {/* New Password */}
            <div>
              <label className="block text-sm text-text-secondary mb-2">{locale['auth.newPassword']}</label>
              <input
                type="password"
                value={password}
                onChange={(e) => setPassword(e.target.value)}
                placeholder={locale['auth.newPasswordPlaceholder']}
                minLength={6}
                className="w-full px-4 py-2.5 rounded-md bg-surface-700 border border-surface-600 text-text-primary placeholder:text-text-muted focus:border-accent focus:ring-1 focus:ring-accent transition-colors"
                required
              />
            ...
            {/* Confirm Password */}
            <div>
              <label className="block text-sm text-text-secondary mb-2">{locale['auth.confirmPassword']}</label>
              <input
                type="password"
            ...
            {(error || localError) && (
              <div className="px-3 py-2 rounded-md bg-danger/10 border border-danger/30">
                <span className="text-sm text-danger">{localError || error}</span>
              </div>
            )}
            ...
            <Button
              ...
              className="w-full"
              loading={isLoading}
              disabled={!email || !code || !password || !confirmPassword}
            >
              {isLoading ? locale['auth.resetting'] : locale['auth.resetPassword']}
            </Button>
          </form>
        )}
